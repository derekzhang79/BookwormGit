...
//    [rightItem release];
    ...
    //为toolbar增加按钮
//    UIBarButtonItem *prePageBtnItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"play-last.png"] style:UIBarButtonItemStylePlain target:self action:@selector(doPre)];
    UIBarButtonItem *fontBtnItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"color.png"] style:UIBarButtonItemStylePlain target:self action:@selector(doFont)];
    UIBarButtonItem *colorBtnItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"light.png"] style:UIBarButtonItemStylePlain target:self action:@selector(doColor)];
//    UIBarButtonItem *nextPageBtnItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"play-next.png"] style:UIBarButtonItemStylePlain target:self action:@selector(doNext)];
    ...
    [self setToolbarItems: [NSArray arrayWithObjects: fontBtnItem,flexItem, colorBtnItem,nil]];
	...
	pageIndex = 1;
	headView = nil;
	CGRect bookRect = [UIScreen mainScreen].applicationFrame;
    bookRect.origin = CGPointZero;
	bookLabel = [[PageView alloc] initWithFrame:bookRect];
	[self.view addSubview:bookLabel];
    
	mBook = [[KDBook alloc]initWithBook:bookIndex];
    mBook.delegate = self;
	mBook.pageSize = bookRect.size;
	mBook.textFont = [UIFont systemFontOfSize:kContentTextSize];
    [mBook startAutoPageSplit];
...
{
    if(mBook.bookSize!=0 && !self.navigationController.navigationBarHidden)
    {
        //self.navigationItem.title= title;
        NSString* title = [NSString stringWithFormat:@"%.2f/100",(double)[mBook offsetWithPage:pageIndex]*100/mBook.bookSize];
        UIFont* font = [UIFont systemFontOfSize:kContentTextSize];
        CGRect rc;
        rc.origin = CGPointZero;
        rc.size = [title sizeWithFont:font
                          constrainedToSize:CGSizeMake(self.view.frame.size.width,self.view.frame.size.height)
                              lineBreakMode:UILineBreakModeWordWrap];
        
        UILabel* titleLabel = [[UILabel alloc] initWithFrame:rc];
