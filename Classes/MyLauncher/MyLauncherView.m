...
static const int maxPageCount = 100;

/* iPhone */
static const int maxItemsPageCount = 9;

static const int portraitItemWidth = 106;
static const int portraitItemHeight = 120;//106;
    ...
    for (NSInteger i = 0; i < capacity; ++i) {
        {
            ...
            for (NSInteger j = 0; j < maxItemsPerPage; ++j)
            {
                NSUInteger index = i*maxItemsPerPage+j;
                if (index>=[pages count]) {
                    break;
                }
                [p addObject:[pages objectAtIndex:index]];
            }
...
-(void)didDeleteItem:(id)item
{
	...
	for (NSMutableArray *page in self.pages)
	{
		...
		for (MyLauncherItem *aitem in page)
		{
			if(aitem == ditem)
			{
                [[NSNotificationCenter defaultCenter]postNotificationName:kDidDeleteItem object:item];
